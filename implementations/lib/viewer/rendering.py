...
def rasterization(
    ...
    render_mode: Literal["RGB", "D", "ED", "RGB+D", "RGB+ED", "NG", "AO", "AS"] = "RGB",
    ...
    assert render_mode in ["RGB", "D", "ED", "RGB+D", "RGB+ED", "NG", "AO", "AS"], render_mode
    ...
    if render_mode in ["RGB+D", "RGB+ED"]:
        ...
        feature = torch.ones_like(opacities)
        colors = feature[..., None]
        if backgrounds is not None:
            backgrounds = torch.zeros(batch_dims + (C, 1), device=backgrounds.device)
    elif render_mode == "AO":
        ...
        avg_scales_feature = (scales.prod(dim=-1)) ** (1.0 / 3.0)
        if packed:
            feature = avg_scales_feature.view(B, N)[batch_ids, gaussian_ids]
        else:
            feature = torch.broadcast_to(
                avg_scales_feature[..., None, :], batch_dims + (C, N)
            )
        colors = feature[..., None]
        if backgrounds is not None:
            backgrounds = torch.zeros(batch_dims + (C, 1), device=backgrounds.device)
