...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, no_fine, render_test=False, reconstruct=False):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        print("Rendering view {} of {}".format(idx, len(views)))
        ...
        print(rendering["deformed_model"].keys())
        deformed_model["means3D"][idx] = rendering["deformed_model"]["means3D"].cpu()
        deformed_model["unnorm_rotations"][idx] = rendering["deformed_model"]["unnorm_rotations"].cpu()
        deformed_model["log_scales"][idx] = rendering["deformed_model"]["log_scales"].cpu()
        deformed_model["logit_opacities"][idx] = rendering["deformed_model"]["logit_opacities"].cpu()
        deformed_model["rgb_colors"][idx] = rendering["deformed_model"]["rgb_colors"].cpu()
        deformed_model["sh"][idx] = rendering["deformed_model"]["sh"].cpu()
    ...
    # Save deformed model as npz to be used by Dynamic 3D Gaussians
    save_dir = os.path.join(model_path, name, "ours_{}".format(iteration))
    os.makedirs(save_dir, exist_ok=True)
    save_path = os.path.join(save_dir, "deformed_model.npz")

    np_deformed_model = {}

    for key in ["means3D", "unnorm_rotations", "rgb_colors", "sh"]:
        # Get list of tensors sorted by timestep
        tensors = [deformed_model[key][i].cpu().numpy() for i in sorted(deformed_model[key].keys())]
        np_deformed_model[key] = np.stack(tensors, axis=0)  # shape: [T, N, ...]

    
    # Save non-time-varying keys directly from first frame
    first_idx = sorted(deformed_model["log_scales"].keys())[0]
    np_deformed_model["log_scales"] = deformed_model["log_scales"][first_idx].cpu().numpy()
    np_deformed_model["logit_opacities"] = deformed_model["logit_opacities"][first_idx].cpu().numpy()

    np.savez(save_path, **np_deformed_model)
    print(f"Saved deformed_model to {save_path}")
